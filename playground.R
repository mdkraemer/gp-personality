...
effects_basic$analysissample <- factor(effects_basic$analysissample, levels = c("LISS Parents", "LISS Nonparents", "HRS Parents", "HRS Nonparents"))
# more intuitive label (appears in legend later)
levels(effects_basic$analysissample) <- c("LISS: Grandparents vs. Parents", "LISS: Grandparents vs. Nonparents", 
                                          "HRS: Grandparents vs. Parents", "HRS: Grandparents vs. Nonparents")

ggplot(effects_basic, aes(x = fct_rev(parameter), y = est, colour = fct_rev(analysissample), group = fct_rev(analysissample))) +
  geom_point(aes(color = fct_rev(analysissample), shape = fct_rev(analysissample)), size=3, position=position_dodge(0.4)) + 
  ...
  guides(colour = guide_legend(reverse = TRUE), shape = guide_legend(reverse = TRUE)) + # known coord_flip bug
...
effects_gender$analysissample <- factor(effects_gender$analysissample, levels = c("LISS Parents", "LISS Nonparents", "HRS Parents", "HRS Nonparents"))
# more intuitive label (appears in legend later)
levels(effects_gender$analysissample) <- c("LISS: Grandparents vs. Parents", "LISS: Grandparents vs. Nonparents", 
                                          "HRS: Grandparents vs. Parents", "HRS: Grandparents vs. Nonparents")

ggplot(effects_gender, aes(x = fct_rev(parameter), y = est, colour = fct_rev(analysissample), group = fct_rev(analysissample))) +
  geom_point(aes(color = fct_rev(analysissample), shape = fct_rev(analysissample)), size=3, position=position_dodge(0.6)) + 
  geom_hline(yintercept=0) +
  geom_errorbar(aes(ymin = lwr, ymax = upr), width=.3, position=position_dodge(0.6)) +
  facet_wrap( ~ outcome, ncol = 6) +
  theme_bw() + 
  scale_shape_manual(values=c(18,17,16,15))+
  scale_color_manual(values=c("#009E73", "#56B4E9", "#E69F00", "#000000"))+
