...
We did not find convincing evidence that life satisfaction changed as a consequence of the transition to grandparenthood. Only in the LISS in comparison with the nonparent control group did grandparents' life satisfaction increase slightly at the first assessment following the transition to grandparenthood. This difference was present in grandmothers but not grandfathers. While this pattern of effects is in line with several studies reporting increases associated with becoming grandparents [e.g., @digessaBecomingGrandparentIts2019; @tanskanenTransitionGrandparenthoodSubjective2019], we did not uncover it reliably in both samples or both comparison groups and also did not see consistent differences in the linear trajectories after the transition to grandparenthood. As mentioned in the introduction, a study into the effects of the transition on first-time grandparents well-being that used fixed effects regressions also did not reveal positive within-person effects of the transition [@sheppardBecomingFirstTimeGrandparent2019]. In line with this study, we did not find evidence that grandparents who provided grandchild care increased more strongly in life satisfaction than those who did not and, likewise, grandparents' life satisfaction trajectories were not moderated by employment status.  
Overall, th accumulated research on life satisfaction indicates that there is an association between having grandchildren and higher life satisfaction on the between-person level---especially for grandmothers who provide frequent grandchild care, but no within-person effect of the transition. One reason for this could be the presence of selection effects, i.e., confounding that we have addressed through the propensity score matching design [@vanderweeleOutcomeWideLongitudinalDesigns2020; @thoemmesSystematicReviewPropensity2011; @luhmannStudyingChangesLife2014].  

## Interindividual Differences in Change

Analyzing how grandparents and matched controls differed interindividually in their trajectories of change provided two additional insights to the analysis of mean-level changes: First, every model's parameters of change exhibited substantial interindividual differences in deviations from the mean-level trajectories as evidenced by significant increases in model fit through the addition of random slopes. Similar to Denissen et al. [-@denissenTransactionsLifeEvents2019] who found significant improvements of random slopes to most of their models' fits [see also @dorePopulationIndividuallevelChanges2018], this indicates that respondents---both grandparents and matched controls---deviated to a considerable extent from the average trajectories that we reported on earlier and that our figures display.[^f16]  
Second, in keeping with our analytical strategy of testing results found in the grandparents against the matched controls we estimated heterogeneous random variance models and tested whether the random slope variance of each change parameter was significantly higher in the grandparent group.  

[^f16]: (internal footnote) If I only look at grandparents and test if adding random slopes to the (simplified) basic model increases model fit significantly I see that now only a minority of model comparisons indicate significant model fit increases (23 out of 72 test; at $\alpha$ = .005; almost none in the HRS - 3 out of 36 tests). Also, a couple of the random slope models now only converge with singular fit (despite BOBYQA optimizer). I think this is because these particular random slopes are estimated as too close to zero. Not sure, if this is just a sample size issue (but why are the HRS random slopes even more often n.s.?) or what is going on. 

## Rank-Order Stability

meaningfulness of small effects in psychology [@gotzSmallEffectsIndispensable2021]

agreeableness: compare with literature on generativity (Erikson)  

strength: internal cross-study replication

grandchild care as possible protective factor agaisnt loneliness in COVID-19 pandemic [@quirkeExtendingUnderstandingGrandchild2020]
...
--> maybe not life transitions but health declines largely responsible for personality development in old age (some evidence for rank-order stability: [@seifertDevelopmentRankOrderStability2021])
